...
| Display | 480x272 framebuffer at 60 Hz on the portrait-mounted panel; `fb0` through `fb3` | X2000 display path | `jzfb` stock framebuffer/display stack | Display controller, panel, clocks, power/backlight, reserved memory | X2000 framebuffer/panel path with Fre3nderScreen Core-UI | QUALIFIED ON DEVICE | The project kernel/DTS/RootFS path produced physical framebuffer output with correct panel colors on the investigated reference system. GPC22 backlight control, automatic startup, 60-second standby, and first-touch wake are physically demonstrated. The previously qualified GuppyScreen pin was persistently deployed and qualified through normal printer control and a complete real print; the current Fre3nderScreen integration candidate is not yet hardware-qualified. See [x2000-display-touch.md](x2000-display-touch.md) and [fre3nderscreen.md](fre3nderscreen.md). [^phase32] |
| Touch | NS2009 at I2C address `0x48` on bus 4 | I2C 4, evdev input | Stock touchscreen driver | I2C controller, NS2009 node, `pendown-gpios`/pinctrl | Project-carried GPL NS2009 driver through evdev into Fre3nderScreen | QUALIFIED ON DEVICE | The investigated reference system enumerated the controller, identified it dynamically as `ns2009_ts`, and emitted raw X/Y events. The previously qualified GuppyScreen pin demonstrated calibrated touch mapping with `display_rotate: 1`, first-touch wake, and audible touch feedback; the current Fre3nderScreen integration candidate is not yet hardware-qualified. See [x2000-display-touch.md](x2000-display-touch.md) and [fre3nderscreen.md](fre3nderscreen.md). [^phase32] |
